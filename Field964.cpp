...
void GameInit(Config & config, Game & game, Hero * hero)
{
...
void GetEnemys(Config & config, Lists * lists, Game & game, Monster & enemy)
{
	std::vector<Object> e = game.lvl->GetObjects("normal");
	for (int i = 0; i < e.size(); i++)
		Lists().entities.push_back(new Enemy(config.NormalEnemy, *game.lvl, e[i].rect.left, e[i].rect.top, enemy.normal_W, enemy.normal_H, "normal"));
}

void StartGame(Config & config, Game & game, Hero * hero, Lists * lists, Info & info)
{
	Clock clock;
	Clock reload_clock;
			...
			if (hero->player->life == true) {
				...
				if (event.key.code == Keyboard::RControl)
				{
					...
					if (hero->player->reload == true) {
						...
						lists->bullets.push_back(new Bullet(config.Bullet, "Bullet", *game.lvl, hero->player->x, hero->player->y, 5, 5, hero->player->dir));
					}
				}
			}
...
void DrawingGame(Config & config, Game & game, Hero * hero, Lists * lists, Info & info, float time)
{
	game.window->clear(Color(77, 83, 140));
...
int main() {
		...
		StartGame(config, *game, hero(), lists, info);
		return 0;
	}